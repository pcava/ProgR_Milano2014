...
biCluster  = makeCluster(spec = c(rep(ip_master, 4) , rep(ip_slave, 4)), port = 11001)
cbind(clusterCall(biCluster, function() {Sys.info()["nodename"]}))
stopCluster(biCluster)


## ----parallel-030, label="localVaio"-------------------------------------
biCluster  = makeCluster(spec = c(rep(ip_master, 4) , rep(ip_slave, 4)))
